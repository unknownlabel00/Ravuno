...
const STRIPE_PRICE_FOUNDING = "price_1U8nEuCXIvaOsF7l2lvD34yd"; // RM19/mo founding-member price
const STRIPE_PRICE_STANDARD = "price_1U8nEzCXIvaOsF7lmkh2XZWp"; // RM29/mo standard price
...
function buildTemplates(p) {
  return p.category === "apparel" ? buildApparelTemplates(p) : buildGenericTemplates(p);
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [itemRows, txnRows, profileRows] = await Promise.all([
  ...
  const preview = useMemo(() => buildTemplates(form), [form]);
  const activeListing = items.find((l) => l.id === activeId) || null;
  const activeTemplates = activeListing ? buildTemplates(activeListing) : preview;
      ...
      <div className="min-h-screen w-full" style={{ background: "#F5F4F0", fontFamily: "'Inter', sans-serif", color: "#17161A" }}>
        ...
        <main className="max-w-5xl mx-auto px-6">
          ...
          <section className="grid sm:grid-cols-3 gap-4 pb-16">
            {[
              { title: "Cross-List", desc: "One product form, five ready-to-paste platform listings." },
              { title: "Stock Ledger", desc: "Search, track, and export your inventory for stock takes." },
              { title: "POS + Sales", desc: "Search-to-sell checkout with discounts, invoices, and reports." },
            ].map((f) => (
              <div key={f.title} className="rounded-xl border p-5" style={{ background: "#FFFFFF", borderColor: "#E4E1D9" }}>
                <h3 className="text-sm font-semibold mb-1">{f.title}</h3>
                <p className="text-xs" style={{ color: "#726F68" }}>{f.desc}</p>
              </div>
            ))}
          </section>
          ...
          <section className="pb-16">
            <h2 className="text-xl font-semibold text-center mb-1">Pricing</h2>
            <p className="text-xs text-center mb-8" style={{ color: "#8B887F" }}>One plan. Cancel anytime.</p>
            <div className="grid sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
              <div className="rounded-xl border p-6" style={{ background: "#FFFFFF", borderColor: "#146658" }}>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "#EEF3F1", color: "#146658" }}>Founding member</span>
                <p className="text-3xl font-bold mt-3 mb-1" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>RM19<span className="text-sm font-normal" style={{ color: "#8B887F" }}>/mo</span></p>
                <p className="text-xs mb-4" style={{ color: "#726F68" }}>Limited-time price for early users, locked in for life.</p>
              ...
              <div className="rounded-xl border p-6" style={{ background: "#FFFFFF", borderColor: "#E4E1D9" }}>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "#F5F4F0", color: "#726F68" }}>Standard</span>
                <p className="text-3xl font-bold mt-3 mb-1" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>RM29<span className="text-sm font-normal" style={{ color: "#8B887F" }}>/mo</span></p>
                <p className="text-xs mb-4" style={{ color: "#726F68" }}>Full access to Cross-List, Stock Ledger, POS and Sales.</p>
                <button onClick={() => { setAuthMode("signup"); setShowAuthForm(true); }} className="w-full py-2.5 rounded-lg text-sm font-semibold border" style={{ borderColor: "#E4E1D9", color: "#17161A" }}>
                  Get started
                </button>
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-6 relative flex flex-col items-center gap-5">
          <div className="absolute right-6 top-6 flex items-center gap-3">
            {profile?.subscription_status === "active" ? (
              <span className="text-xs font-semibold px-2 py-1 rounded-full" style={{ background: "#EEF3F1", color: "#146658" }}>Pro</span>
            ) : (
              <button onClick={() => handleUpgrade(STRIPE_PRICE_FOUNDING)} disabled={upgradeBusy}
                className="text-xs font-semibold px-3 py-1.5 rounded-lg" style={{ background: "#146658", color: "#FFFFFF", opacity: upgradeBusy ? 0.6 : 1 }}>
                {upgradeBusy ? "…" : "Upgrade to Pro — RM19/mo"}
              </button>
            )}
          ...
          <nav className="flex gap-1 p-1 rounded-lg" style={{ background: "#F5F4F0", border: "1px solid #E4E1D9" }}>
            {[{ id: "crosslist", label: "Cross-List" }, { id: "inventory", label: "Stock Ledger" }, { id: "pos", label: "POS" }, { id: "sales", label: "Sales" }].map((t) => (
