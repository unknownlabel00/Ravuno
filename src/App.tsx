...
function TemplatesTab({ customTemplates, onSave, saving, logoUrl, onLogoUpload, onLogoRemove, logoUploading }) {
...
export default function App() {
  ...
  async function completeSale() {
    ...
    const txnRow = txnToRow({
      ...
      lines: cartLines.map((l) => ({ title: productTitle(l), sku: l.sku, price: l.price, discount: l.discount, finalPrice: l.finalPrice })),
    }, session.user.id);
    try {
        ...
        <header style={{ background: "#17171A", borderBottom: "1px solid #2A2A2F" }}>
          <div className="max-w-5xl mx-auto px-6 py-5 flex items-center justify-between">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[10px] font-medium tracking-wider" style={{ color: "#75726C" }}>V1.1</span>
            </div>
    ...
    <div className="min-h-screen w-full" style={{ background: "#0B0B0C", color: "#F2F0EA", fontFamily: "'Inter', sans-serif" }}>
      ...
      <header style={{ background: "#17171A", borderBottom: "1px solid #2A2A2F" }}>
        <div className="max-w-6xl mx-auto px-6 py-6 relative flex flex-col items-center gap-5">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-semibold tracking-tight" style={{ color: "#F2F0EA", fontFamily: "'Fraunces', serif" }}>RAVUNO</span>
            <span className="text-[10px] font-medium tracking-wider" style={{ color: "#75726C", fontFamily: "'IBM Plex Mono', monospace" }}>V1.1</span>
          </div>
        ...
        <main className="max-w-6xl mx-auto px-6 py-8">
          ...
          {tab === "templates" && (
            <TemplatesTab customTemplates={customTemplates} onSave={saveTemplates} saving={templatesSaving}
              logoUrl={logoUrl} onLogoUpload={handleLogoUpload} onLogoRemove={handleLogoRemove} logoUploading={logoUploading} />
          )}
